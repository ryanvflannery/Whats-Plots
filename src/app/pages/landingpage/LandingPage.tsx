    ...
    <>
      <section className="w-full px-8 py-40 grid grid-cols-1 md:grid-cols-2 items-center gap-8 max-w-6xl mx-auto">
        <div>
          ...
          <button className="bg-white text-black font-medium py-2 mt-4 px-4 rounded transition-all hover:bg-gray-100 active:scale-95">
            <SignInButton />
          </button>
        </div>
        <ShuffleGrid />
...
const Card = ({ title, subtitle, Icon, href }: CardType) => {
  return (
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-r from-gray-600 to-black-600 translate-y-[100%] group-hover:translate-y-[0%] transition-transform duration-300" />
...
const ShuffleGrid = () => {
  ...
  return (
    <div className="grid grid-cols-4 grid-rows-4 h-[250px] gap-1">
      {squares.map((sq) => sq)}
    </div>
  );
};
